...
export function handleGetSoilClassification(db: Database, args: SoilArgs) {
  ...
  if (args.soil_type) {
    ...
    return {
      ...
      _citation: buildCitation(
        `NL Soil — ${soil.name || args.soil_type}`,
        `Dutch soil classification for ${soil.name || args.soil_type}`,
  ...
  if (args.texture) {
    ...
    return {
      ...
      _meta: buildMeta(),
      _citation: buildCitation(
        `NL Soil Texture — ${args.texture}`,
        `Dutch soil classification by texture ${args.texture}`,
        'get_soil_classification',
        { texture: args.texture! },
      ),
    };
  ...
  return {
    ...
    _meta: buildMeta(),
    _citation: buildCitation(
      `NL Soil Classifications`,
      `Dutch soil classification — all types`,
      'get_soil_classification',
      {},
    ),
  };
